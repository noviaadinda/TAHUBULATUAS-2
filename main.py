...
def tambah():
    conn = sqlite3.connect('cars.db')
    c = conn.cursor()
    c.execute("""INSERT INTO cars (ID, BRAND, MODEL, PRICE) VALUES ("180", "Toyota", "Ayla", "900")""")
    conn.commit()
    c.close()
...
def tambah():
    conn = sqlite3.connect('cars.db')
    c = conn.cursor()
    c.execute("""UPDATE cars SET MODEL='Accord' WHERE BRAND='Honda' """)
    conn.commit()
    c.close()
...
def tambah():
    conn = sqlite3.connect('cars.db')
    c = conn.cursor()
    c.execute("""DELETE FROM cars WHERE PRICE='130'""")
    conn.commit()
    c.close()
...
def addNEW():
    ...
    if nid != "":        
        conn = sqlite3.connect('cars.db')
        c = conn.cursor()
        c.execute("""INSERT INTO cars (ID, BRAND, MODEL, PRICE) VALUES (nid, nbrand, nmodel, nprice)""")
        conn.commit()
        c.close()
...
@route('/json')
def show_json():
    return '<h1> Ini Halaman menampilkan data .json 7B2 TAHUBULAT</h1>' + template('db/Cars.json')

@route('/xml')
def show_xml():
    return '<h1> Ini Halaman menampilkan data .xml 7B2 TAHUBULAT</h1>' + template('db/Cars.xml') 

@route('/csv')
def show_csv():
    return '<h1> Ini Halaman menampilkan data .csv 7B2 TAHUBULAT</h1>' + template('db/Cars.csv')
